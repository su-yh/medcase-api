...
public enum ErrorCodeEnums implements IErrorCode {
    ...
    DOCTOR_REGISTER_USERNAME_EMPTY("error.code.doctor.register.username.empty", "医生注册用户名不能为空"),
    DOCTOR_REGISTER_PASSWORD_EMPTY("error.code.doctor.register.password.empty", "医生注册密码不能为空"),
    DOCTOR_REGISTER_USERNAME_LENGTH_INVALID("error.code.doctor.register.username.length.invalid", "医生注册用户名长度必须在2到20个字符之间"),
    DOCTOR_REGISTER_PASSWORD_LENGTH_INVALID("error.code.doctor.register.password.length.invalid", "医生注册密码长度必须在5到20个字符之间"),
    ...
    DOCTOR_REGISTER_TITLE_EMPTY("error.code.doctor.register.title.empty", "职称不能为空"),
    DOCTOR_REGISTER_INVITE_CODE_EMPTY("error.code.doctor.register.invite.code.empty", "邀请码不能为空"),
    DOCTOR_REGISTER_INVITE_CODE_INVALID("error.code.doctor.register.invite.code.invalid", "邀请码错误"),
    DOCTOR_REGISTER_ID_CARD_FRONT_EMPTY("error.code.doctor.register.id.card.front.empty", "身份证正面图片不能为空"),
    DOCTOR_REGISTER_ID_CARD_BACK_EMPTY("error.code.doctor.register.id.card.back.empty", "身份证反面图片不能为空"),
    DOCTOR_REGISTER_QUALIFICATION_CERTIFICATE_EMPTY(
            "error.code.doctor.register.qualification.certificate.empty", "医师职业资格证图片不能为空"),
    DOCTOR_LOGIN_USER_NOT_EXISTS("error.code.doctor.login.user.not.exists", "医生账号不存在"),
    DOCTOR_CASE_NOT_FOUND("error.code.doctor.case.not.found", "病例不存在"),
    DOCTOR_CASE_SUBMIT_FAILED("error.code.doctor.case.submit.failed", "病例提交失败，请稍后重试"),
    DOCTOR_LOGIN_FAILED("error.code.doctor.login.failed", "医生登录失败，请检查账号和密码"),
    DOCTOR_UPDATE_REJECT("error.code.doctor.update.reject", "只允许修改自己的病例"),
    DOCTOR_UPDATE_STATUS_NOT_MATCH("error.code.doctor.update.status.not.match", "只允许修改草稿病例"),
    USER_TYPE_NOT_MATCH("error.code.user.type.not.match", "禁止访问，用户类型错误"),
    DOCTOR_DELETE_STATUS_NOT_MATCH("error.code.doctor.delete.status.not.match", "只允许删除草稿病例"),
    DOCTOR_CASE_DELETE_FAILED("error.code.doctor.case.delete.failed", "病例删除失败，请稍后重试"),
    DOCTOR_CASE_REVIEW_STATUS_NOT_MATCH("error.code.doctor.case.review.status.not.match", "只允许审核待审核病例"),
    DOCTOR_CASE_REVIEW_REASON_EMPTY("error.code.doctor.case.review.reason.empty", "审核拒绝原因不能为空"),
    DOCTOR_CASE_REVIEW_FAILED("error.code.doctor.case.review.failed", "病例审核失败，请稍后重试"),
    DOCTOR_CASE_REVIEW_STATUS_INVALID("error.code.doctor.case.review.status.invalid", "审核状态无效"),
    DOCTOR_CASE_SETTLE_STATUS_NOT_MATCH("error.code.doctor.case.settle.status.not.match", "只允许结算审核通过的待结算病例"),
    DOCTOR_CASE_SETTLE_FAILED("error.code.doctor.case.settle.failed", "病例结算失败，请稍后重试"),
    DOCTOR_USER_NOT_FOUND("error.code.doctor.user.not.found", "医生用户不存在"),
    DOCTOR_USER_REVIEW_STATUS_NOT_MATCH("error.code.doctor.user.review.status.not.match", "只允许审核待审核医生"),
    DOCTOR_USER_REVIEW_REASON_EMPTY("error.code.doctor.user.review.reason.empty", "审核拒绝原因不能为空"),
    DOCTOR_USER_REVIEW_FAILED("error.code.doctor.user.review.failed", "医生审核失败，请稍后重试"),
    DOCTOR_LOGIN_REVIEW_FAILED("error.code.doctor.login.review.failed", "医生审核未通过，请使用原账号重新提交审核"),
    DOCTOR_CASE_DOCTOR_STATUS_NOT_OK("error.code.doctor.case.doctor.status.not.ok", "医生资料审核通过后才可操作病例"),
    DOCTOR_PROFILE_SUBMIT_STATUS_NOT_MATCH("error.code.doctor.profile.submit.status.not.match", "当前医生状态不允许提交资料"),
    DOCTOR_PROFILE_SUBMIT_FAILED("error.code.doctor.profile.submit.failed", "医生资料提交失败，请稍后重试"),
    DOCTOR_ACCOUNT_DELETE_STATUS_NOT_MATCH("error.code.doctor.account.delete.status.not.match", "当前医生状态不允许删除账号"),
    DOCTOR_ACCOUNT_DELETE_FAILED("error.code.doctor.account.delete.failed", "当前医生状态不允许删除账号"),
    ;
