...
public enum ErrorCodeEnums implements IErrorCode {
    ...
    USER_REGISTER_SUPPLIER_INVALID("error.code.user.register.supplier.invalid", "邀请人不存在或已停用"),
    SUPPLIER_NICKNAME_EXISTS("error.code.supplier.nickname.exists", "供应商昵称已存在"),
    SUPPLIER_NOT_FOUND("error.code.supplier.not.found", "供应商不存在"),
    SUPPLIER_STATUS_INVALID("error.code.supplier.status.invalid", "供应商状态无效"),
    SUPPLIER_OPERATION_FAILED("error.code.supplier.operation.failed", "供应商操作失败"),
    SUPPLIER_STATUS_UPDATE_FAILED("error.code.supplier.status.update.failed", "供应商状态更新失败"),
