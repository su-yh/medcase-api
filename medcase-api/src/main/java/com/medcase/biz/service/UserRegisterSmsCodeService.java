...
public class UserRegisterSmsCodeService {
    ...
    public void verifyCode(String phone, String code) {
        ...
        if (!StringUtils.hasText(code)) {
            throw ExceptionUtil.business(ErrorCodeEnums.USER_REGISTER_SMS_CODE_EMPTY);
        }

        if (!smsProperties.isEnabled()) {
            if (!FIXED_SMS_CODE.equals(code)) {
                throw ExceptionUtil.business(ErrorCodeEnums.USER_REGISTER_SMS_CODE_INVALID);
            }
            return;
        }
        ...
        if (expectedCode == null) {
            throw ExceptionUtil.business(ErrorCodeEnums.USER_REGISTER_SMS_CODE_EXPIRED);
        }
    ...
    private void validatePhone(String phone) {
        if (!StringUtils.hasText(phone)) {
            throw ExceptionUtil.business(ErrorCodeEnums.USER_REGISTER_PHONE_EMPTY);
        }
        if (!MOBILE_PHONE_PATTERN.matcher(phone).matches()) {
            throw ExceptionUtil.business(ErrorCodeEnums.USER_REGISTER_PHONE_INVALID);
        }
    }

    private String getCodeKey(String phone) {
        return CacheConstants.USER_REGISTER_SMS_CODE_KEY + phone;
    }

    private String getCooldownKey(String phone) {
        return CacheConstants.USER_REGISTER_SMS_COOLDOWN_KEY + phone;
    }
}
